...
| Transport Resource                     | `vehicles`, `vehicle_amenities`, `seat_maps`, `seats`, `routes`, `route_stops`, `stop_points`                    | `vehicle/`, `route/`, `stop-point/`                                | Operator-owned, BẮT BUỘC `operator_id` + RLS. Seat layout config dùng `JSONB`. Toạ độ stop-point cache distance/duration Goong (ADR-027)                                                                                                                            |
