...
| `stop_points` / `stop_point_proposals` | Điểm riêng: unique `(operator_id, name)`, FK ghép phường–tỉnh, CHECK toạ độ + RLS. Đề xuất: CHECK trạng thái nhất quán (`REJECTED` có lý do, `APPROVED` có `catalog_stop_point_id`); RLS cho tenant chỉ tạo/ghi `PENDING` và chỉ sửa bản `REJECTED` | Điểm riêng dùng ngay trong tenant; Operator không tự duyệt được đề xuất (TASK-TRN-002) |
| `routes` / `route_stops` | `routes` unique `(operator_id, name)`; `route_stops` unique `(route_id, sequence)`, CHECK đúng một nguồn (`catalog_stop_point_id` XOR `stop_point_id`), không lặp điểm trong route, FK ghép `(route_id, operator_id)` và `(stop_point_id, operator_id)` + RLS | Route theo thứ tự; khoảng cách/thời gian Goong lưu trên từng chặng (ADR-027) |
| `seat_maps` / `seats` | `seat_maps` unique `(operator_id, name)`; `seats` unique `(seat_map_id, code)` + `(seat_map_id, deck, row, column)`, FK ghép `(seat_map_id, operator_id)` + RLS | Sơ đồ ghế do nhà xe tự cấu hình, dùng chung nhiều xe |
